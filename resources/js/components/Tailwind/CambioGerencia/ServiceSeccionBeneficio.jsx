...
const ServiceSeccionBeneficio = () => {
  return (
    <section className="w-full bg-primary bg-cover bg-center py-16 px-[5%] min-h-[100vh] flex flex-col items-center justify-center relative" style={{backgroundImage: 'url(/assets/cambiogerencia/bg-beneficios.png), linear-gradient(0deg, #15506A 0%, #15506A 100%)'}}>
      <div className="max-w-6xl w-full mx-auto">
        <div className="text-center mb-12">
          <div className="flex justify-center items-center gap-2 mb-2">
            ...
            <span className="uppercase text-white text-lg font-bold tracking-wide">Beneficios</span>
          </div>
          <h2 className="text-4xl lg:text-5xl font-bold mb-4 leading-tight text-white">
            Beneficios de trabajar<br className="hidden md:block" /> con <span className="text-accent italic">Culture 360</span>
          </h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {beneficios.map((b, i) => (
            <div key={i} className="bg-white/5 rounded-2xl p-7 flex flex-col h-full min-h-[200px] shadow-md backdrop-blur-sm border border-white/10">
              <div className="mb-4">
                {b.icon === "stack" ? <IconStack /> : <IconUsers />}
              </div>
              <h3 className="text-white text-xl font-semibold mb-2 leading-snug">{b.title}</h3>
              <p className="text-white/80 text-base font-normal">{b.desc}</p>
            </div>
          ))}
        </div>
      </div>
