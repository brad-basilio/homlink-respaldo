...
const ServiceSeccionMetodologia = ({service}) => {
    // --- NUEVA LÓGICA DE STEPS ANIMADOS ---
    const steps = service?.steps_methodology || [];
...
{service?.description_methodology}
                ...
                {/* NUEVO TIMELINE ANIMADO */}
                <div className="relative max-w-2xl p-4 lg:w-auto overflow-x-hidden overflow-y-visible">
                  <div className="relative">
                    {/* Vertical line */}
                    <div className="absolute left-[18px] top-[36px] bottom-0 w-0.5 bg-red-300"></div>

                    {steps.length > 0 && steps.map((step, index) => (
                      <div key={index} className="relative flex items-start mb-12 last:mb-0">
                        {/* Step circle */}
                        <div
                          className={`relative z-10 flex items-center justify-center w-9 h-9 rounded-full transition-all duration-500 ${
                            index === currentStep ? "bg-red-500" : index < currentStep ? "bg-red-300" : "bg-red-200"
                          }`}
                        >
                         <img src={`/api/service/media/${step.image}`} alt={`Icono del paso ${index + 1}`} className="w-5 h-5" />
                        </div>

                        {/* Content */}
                        <div className="ml-6 flex-1">
                          <div className="text-sm font-medium text-gray-600 mb-1">PASO {index + 1}</div>
                          <h3 className="text-2xl font-bold text-gray-900 mb-3">{step.title}</h3>
                          <div
                            className={`transition-all duration-500 overflow-hidden ${
                              index <= currentStep ? "max-h-20 opacity-100" : "max-h-0 opacity-0"
                            }`}
                          >
