...
const EmpresasSection = ({ banner_slider }) => {
    ...
    if (!banner_slider || banner_slider.length === 0) {
        return null; // O mostrar contenido por defecto
    }

    const currentBanner = banner_slider[currentSlide];

    return (
        <section className="w-full bg-primary py-32 flex justify-center items-center font-title px-[5%] mx-auto">
            <div className="relative  w-full px-16 rounded-[56px] bg-constrast  flex flex-col md:flex-row items-center  py-10 min-h-[380px]">
                ...
                <div className="z-10 flex-1 flex justify-center items-end">
                    <img 
                        ...
                        className="h-[600px] object-cover absolute bottom-0 select-none transition-opacity duration-500" 
                        draggable="false" 
                    />
                </div>
